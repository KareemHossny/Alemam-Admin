...
const AddProject = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage(err.message || 'Error creating project. Please try again.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 sm:mb-8 gap-4">
        <div className="flex items-center space-x-3 sm:space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-600 text-sm sm:text-base">Add a new project to the system</p>
          </div>
        </div>
          ...
          {message && (
            <div className={`p-3 sm:p-4 rounded-xl sm:rounded-2xl border ${
              ...
              <div className="flex items-center space-x-2 sm:space-x-3">
                {message.includes('successfully') ? (
                  <FiPlus className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-600" />
                ) : (
