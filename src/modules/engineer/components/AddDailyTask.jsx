...
const AddDailyTask = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const bulkResult = await engineerAPI.addDailyTasksBulk(
        validTasks.map((task) => ({
          projectId,
          title: task.title,
          note: task.note,
      ...
      setMessage(`${createdCount} daily task(s) added successfully!`);
      setTasks([{ title: '', note: '' }]);
      
      setTimeout(() => {
        navigate('/engineer/add-tasks');
      }, 2000);
      
    } catch (err) {
      const fallbackMessage = err.message || 'Failed to add daily tasks';
    ...
    <div className="max-w-4xl mx-auto w-full px-2 sm:px-4">
      ...
      <div className="flex items-center justify-between mb-6 sm:mb-8">
        <div className="flex items-center gap-3 sm:gap-4">
          <button
            onClick={() => navigate('/engineer/add-tasks')}
