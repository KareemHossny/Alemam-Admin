...
const EngineerDashboard = ({ onLogout, engineerInfo }) => {
  ...
  const menuItems = [
    {
      path: '/engineer/add-tasks',
      label: 'Add Tasks',
      icon: FiPlus,
      description: 'Add daily or monthly tasks'
    },
    {
      path: '/engineer/view-tasks',
      label: 'View Tasks',
      icon: FiEye,
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3 sm:gap-4">
                <div className="w-10 h-10 sm:w-14 sm:h-14 bg-gradient-to-r from-blue-600 to-indigo-700 rounded-xl sm:rounded-2xl flex items-center justify-center text-white font-bold text-sm sm:text-lg shadow-xl">
                  <FiUser className="w-4 h-4 sm:w-6 sm:h-6" />
                </div>
                <div className="flex flex-col">
                  <h1 className="text-lg sm:text-2xl font-extrabold text-gray-800 leading-tight">Engineer Portal</h1>
                  <p className="text-xs sm:text-sm text-gray-500 mt-1 font-medium">Task Management System</p>
                </div>
              </div>
              <button
      ...
      <div className="max-w-5xl mx-auto">
        <div className="bg-white/90 backdrop-blur-md rounded-2xl sm:rounded-3xl shadow-xl border border-gray-200/60 p-4 sm:p-6 lg:p-8">
          <h3 className="text-xl sm:text-2xl lg:text-3xl font-extrabold text-gray-800 mb-4 sm:mb-6 lg:mb-8 flex items-center gap-2 sm:gap-3 lg:gap-4">
            <FiBarChart2 className="w-5 h-5 sm:w-6 sm:h-6 lg:w-8 lg:h-8 text-blue-600" />
            Quick Actions
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            <Link
              to="/engineer/add-tasks"
              className="flex items-center gap-3 sm:gap-4 p-4 sm:p-6 rounded-xl sm:rounded-2xl border-2 border-gray-200/60 hover:border-blue-400 hover:shadow-xl transition-all duration-300 group bg-white/60 hover:bg-blue-50 font-semibold text-gray-800 hover:text-blue-700"
            >
              <div className="w-12 h-12 sm:w-14 sm:h-14 lg:w-16 lg:h-16 bg-blue-100 rounded-xl sm:rounded-2xl flex items-center justify-center group-hover:bg-blue-200 transition-colors">
                <FiPlus className="w-5 h-5 sm:w-6 sm:h-6 lg:w-8 lg:h-8 text-blue-600 group-hover:text-blue-700 transition-colors" />
              </div>
              <div className="text-left flex-1">
                <div className="text-lg sm:text-xl lg:text-2xl font-bold transition-colors">Add New Tasks</div>
                <div className="text-gray-600 text-sm sm:text-base lg:text-lg mt-1">Create daily or monthly tasks</div>
              </div>
            </Link>
            
            <Link
              to="/engineer/view-tasks"
              className="flex items-center gap-3 sm:gap-4 p-4 sm:p-6 rounded-xl sm:rounded-2xl border-2 border-gray-200/60 hover:border-blue-400 hover:shadow-xl transition-all duration-300 group bg-white/60 hover:bg-blue-50 font-semibold text-gray-800 hover:text-blue-700"
            >
              ...
              <div className="text-left flex-1">
                <div className="text-lg sm:text-xl lg:text-2xl font-bold transition-colors">View My Tasks</div>
                <div className="text-gray-600 text-sm sm:text-base lg:text-lg mt-1">Monitor task progress</div>
              </div>
            </Link>
          </div>
        </div>
